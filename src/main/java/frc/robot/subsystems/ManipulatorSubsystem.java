...
public class ManipulatorSubsystem extends SubsystemBase {
  ...
  private final TalonFXS m_manipulatorDriveMotor =
      new TalonFXS(CANConstants.kManipulatorDriveMotorID);
  // Defines a TalonFX motor which is connected to a Falcon 500.
  private final TalonFX m_manipulatorAngleMotor =
      new TalonFX(CANConstants.kManipulatorAngleMotorID);
  ...
  /** Creates a Manipulator Subsystem This is used to control the coral manipulator. */
  ...
  /**
   * Creates and Applies Motor Configurations This creates and applies the motor configurations for
   * the manipulator's drive motor and its turn motor. Along with this, it creates and configures
   * parameters to be used with motion magic.
   */
  public void applyMotorConfigurations() {
    ...
    m_manipulatorDriveMotor.getConfigurator().apply(m_manipulatorDriveConfiguration, 0.050);
    m_manipulatorAngleMotor.getConfigurator().apply(m_manipulatorAngleConfiguration, 0.050);
   ...
   *
   * @param speed percentage value [-1, 1] used to control the manipulator's drive motor
   * @return Returns a new RunCommand used to set the speed percentage for the manipulator's drive
   *     motor.
   */
  public Command getManipulatorDriveCommand(double speed) {
    ...
    return new RunCommand(() -> m_manipulatorDriveMotor.setControl(manipulatorSpeed), this);
  }
